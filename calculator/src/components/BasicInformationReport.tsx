    ...
    <div style={{ fontSize: 13, lineHeight: 1.4 }}>
      ...
      <StatGrid>
        ...
        <Stat
          ...
          color="#1b5e20"
          note="Definition 1 — click Section 3 for detail"
        />
      </StatGrid>
      ...
      <KV
        ...
        value={i.vehicleCondition === "Used – private sale (no GST)"
          ...
          : `$${aud(vehicleGstSaved)}`}
      />
      <KV label="Amount financed (ex GST)" value={`$${aud(amountFinanced)}`}
      ...
      <KV label={`ATO-mandated residual % (${Math.round(i.leaseDurationYears)} years)`}
        tooltip={<InfoTooltip text={<><p style={{ margin: "0 0 6px 0" }}>ATO statutory residual values:</p><ul style={{ margin: 0, paddingLeft: 18 }}><li>1y → 65.63%</li><li>2y → 56.25%</li><li>3y → 46.88%</li><li>4y → 37.50%</li><li>5y → 28.13%</li></ul></>} />}
        value={pct(residualPct)} />
      <KV label={`Residual payable after ${Math.round(i.leaseDurationYears)} years (inc GST)`}
        tooltip={<InfoTooltip text="The amount you pay at lease end to own the vehicle, including GST." />}
        value={`$${aud(residualPayableIncGst)}`} />
      <KV
        label={
          <button type="button" onClick={() => navigateTo("details-section-3-effective-interest-rate")}
      ...
      {/* ── ECM section (FBT-applicable only) ─────────────────── */}
      {fbtApplicable && (
        <div style={{ marginTop: 16, background: "rgba(230,81,0,0.04)", border: "1px solid rgba(230,81,0,0.18)", borderRadius: 10, padding: "10px 14px" }}>
          ...
          <StatGrid>
            <Stat label="Annual ECM (post-tax)" value={`$${aud(ecmAnnual)}`} color="#e65100"
              note={`${Math.round(fbtStatutoryRate * 100)}% of dutiable value`} />
            <Stat label="ECM per fortnight" value={`$${aud(ecmPerFn)}`} color="#e65100" />
            <Stat label="FBT delta per fortnight" value={`$${aud(fbtDeltaPerFn)}`} color="#b71c1c"
              note="Extra cost vs FBT-exempt EV" />
          </StatGrid>
          ...
          <KV label="FBT delta (fortnight)" bold highlight color="#b71c1c"
      ...
      {i.vehicleType === "EV" && (
        <div style={{ marginTop: 16, background: "rgba(245,124,0,0.04)", border: "1px solid rgba(245,124,0,0.18)", borderRadius: 10, padding: "10px 14px" }}>
          ...
          <KV label="Actual charging expense per year" value={`$${aud(chargingExpensePerYear)}`}
            tooltip={<InfoTooltip text="Real out-of-pocket cost based on your tariff." />} />
          <KV label="NL claim amount per year" value={`$${aud(assumedChargingClaimPerYear)}`}
            tooltip={<InfoTooltip text="The packaged electricity amount. May default to ATO 5.47c/km shortcut — you can override in the Inputs panel." />} />
          <KV label="Charging delta" value={`$${aud(chargingDelta)}`}
          ...
          <KV label="Net charging expense (after tax reimbursement)" value={`$${aud(postReimbursementEffectiveChargingExpense)}`}
            bold highlight color={postReimbursementEffectiveChargingExpense <= 0 ? "#1b5e20" : "#b71c1c"}
            tooltip={<InfoTooltip text={<><p style={{ margin: "0 0 8px 0" }}>Actual expense − (claim × marginal tax rate). Negative = net gain.</p><p style={{ margin: 0 }}><b>Example (default):</b> $371 actual − ($821 × 47%) tax benefit = −$14 net (a small gain).</p></>} />} />
          {chargingDelta < 0 && (
