...
/** Ported from calculator/src/components/WhatIf.tsx — same maths/structure. */
export function WhatIf({ inputs, vehicleLeasePeriodMode }: WhatIfProps) {
  ...
  const { hypotheticalPerFn, error } = useMemo(() => {
    try {
      ...
      return { hypotheticalPerFn: 0, error: e instanceof Error ? e.message : String(e) };
    }
    ...
    <div style={{ fontSize: 13, lineHeight: 1.5 }}>
        ...
        <table style={{ width: "100%", minWidth: "max-content", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr>
              ...
              <th style={thR}>Per {period}</th>
              <th style={thR}>Lifetime total</th>
            </tr>
          </thead>
          <tbody>
            <tr>
      ...
      <div style={{ fontSize: 12.5, lineHeight: 1.55, color: "var(--nlc-text-soft)" }}>
        <p style={{ margin: "0 0 8px 0" }}>
          ...
          spread between wholesale and quoted interest rates, administration fees, and commissions from bundled products
          such as insurance and service packages — and it is generally impossible to determine their true total revenue or
          how it is split across these channels.
        </p>
        <p style={{ margin: 0 }}>
          <b>Note:</b> This shows the <b>pre-tax</b> repayment difference only. To approximate the net after-tax impact,
          enter <b>{money(fnToCol(hypotheticalPerFn2dp))} per {period}</b> as the vehicle lease in the Inputs panel and
          compare Summary outcomes.
        </p>
      </div>
    </div>
  );
